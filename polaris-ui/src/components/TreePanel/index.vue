<template>
  <div :class="{ collapsed: collapsed, resizing: isResizing, 'no-initial-transition': isLoadingFromStorage}" :style="{ width: sidebarWidth + 'px' }" class="tree-sidebar">
    <!-- 右侧拖动条 -->
    <div v-if="!collapsed" :class="{ active: isResizing }" class="resize-handle" @mousedown="startResize" @touchstart="startResize" />
    <div class="tree-header">
      ...
      <div v-show="!collapsed" class="tree-actions">
        <el-tooltip :content="isExpandedAll ? '收起全部' : '展开全部'" placement="right">
          <i :class="isExpandedAll ? 'el-icon-arrow-down' : 'el-icon-arrow-up'" class="tree-action-icon" @click="toggleExpandAll" />
        </el-tooltip>
    ...
    <div class="collapse-button-container">
      <el-tooltip :content="collapsed ? '展开' : '收起'" placement="right">
        <i :class="collapsed ? 'el-icon-d-arrow-right' : 'el-icon-d-arrow-left'" class="collapse-button" @click="toggleCollapsed" />
      </el-tooltip>
    </div>

    <div v-if="showSearch" v-show="!collapsed" class="tree-search">
      <el-input v-model="searchKeyword" :placeholder="searchPlaceholder" clearable prefix-icon="el-icon-search" size="small" @input="onSearch" />
    </div>
    ...
    <div v-show="!collapsed" class="tree-wrap">
      ...
      >
        <span slot-scope="{ node, data }" class="tree-node">
          <slot :data="data" :node="node" name="node">
