    ...
    <div class="filter-container">
      <el-form :inline="true" class="demo-form-inline" size="small">
        <el-form-item label="模型名称">
          <el-input v-model="queryParams.name" clearable placeholder="请输入配置名称" @keyup.enter.native="handleQuery"/>
        </el-form-item>
        ...
        <el-form-item>
          ...
          <el-button class="btn-gradient-success" icon="el-icon-plus" type="success" @click="handleAdd">新增模型</el-button>
        </el-form-item>
      </el-form>
    </div>
        ...
        <el-col v-for="item in modelList" :key="item.id" :lg="8" :md="8" :sm="12" :xs="24" class="card-col">
          <div :class="{ 'is-active-chat': item.isDefault === '1', 'is-active-embedding': item.isDefaultEmbedding === '1' }" class="model-card">
            ...
            <div class="card-header">
              <div :class="item.provider" class="provider-avatar">
              ...
              <div class="header-info">
                ...
                <span :class="item.provider" class="provider-badge">{{ getProviderLabel(item.provider) }}</span>
              </div>
            ...
            <div class="card-body">
              <div class="param-row">
                <span class="param-label"><i class="el-icon-cpu"></i> 模型名称</span>
                <span class="param-val code-text">{{ item.modelName }}</span>
              </div>
              <div class="param-row">
                <span class="param-label"><i class="el-icon-key"></i> API Key</span>
                <span class="param-val code-text mask-text">{{ item.apiKey ? '••••••••••••••••' : '未设置' }}</span>
              </div>
              <div v-if="item.baseUrl" class="param-row">
                <span class="param-label"><i class="el-icon-link"></i> API 地址</span>
              ...
              <div class="param-row">
                <span class="param-label"><i class="el-icon-chat-line-round"></i> 思考模式</span>
                <span class="param-val">
                  <el-tag v-if="item.enableThinking === '1'" effect="plain" size="mini" type="warning">
                    开启 ({{ getReasoningEffortLabel(item.reasoningEffort) }})
                  </el-tag>
                  <el-tag v-else effect="plain" size="mini" type="info">关闭</el-tag>
                </span>
              </div>
              <div class="param-row">
                <span class="param-label"><i class="el-icon-connection"></i> 联网搜索</span>
                <span class="param-val">
                  <el-tag v-if="item.enableSearch === '1'" effect="plain" size="mini" type="success">开启</el-tag>
                  <el-tag v-else effect="plain" size="mini" type="info">关闭</el-tag>
                </span>
              </div>
              <div class="param-row">
                <span class="param-label"><i class="el-icon-office-building"></i> 归属部门</span>
                <span class="param-val">
                  <el-tag v-if="item.deptId" effect="plain" size="mini" type="primary">
                    {{ getDeptName(item.deptId) }}
                  </el-tag>
                  <el-tag v-else effect="plain" size="mini" type="success">全局共享</el-tag>
                </span>
              </div>
            ...
            <div class="card-footer">
              <div class="default-tags">
                <template v-if="!isEmbeddingModel(item.modelName)">
                  <el-tag v-if="item.isDefault === '1'" class="active-tag-chat" effect="dark" size="mini" type="success">
                    <i class="el-icon-chat-dot-round"></i> 默认聊天
                  </el-tag>
                  <el-button v-else class="footer-action-btn" size="mini" type="text" @click="handleSetDefaultChat(item)">
                    设为默认聊天
                  </el-button>
                </template>

                <template v-if="isEmbeddingModel(item.modelName)">
                  <el-tag v-if="item.isDefaultEmbedding === '1'" class="active-tag-embed" effect="dark" size="mini" type="primary">
                    <i class="el-icon-collection"></i> 默认向量
                  </el-tag>
                  <el-button v-else class="footer-action-btn color-primary" size="mini" type="text" @click="handleSetDefaultEmbedding(item)">
                    设为默认向量
                  </el-button>
                </template>
              </div>
              ...
              <div class="action-buttons">
                <el-button class="footer-icon-btn edit-btn" icon="el-icon-edit" size="mini" type="text" @click="handleUpdate(item)">编辑</el-button>
                <el-button class="footer-icon-btn delete-btn" icon="el-icon-delete" size="mini" type="text" @click="handleDelete(item)">删除</el-button>
              </div>
            </div>
    ...
    <el-dialog :title="title" :visible.sync="open" append-to-body custom-class="polaris-form-dialog" class="ai-model-dialog" width="640px">
      <el-form ref="form" :model="form" :rules="rules" label-width="110px" size="small">
        <el-row>
          ...
          <el-col v-if="form.enableSearch === '1'" :span="24">
...
.provider-avatar.deepseek {
  background: rgba(59, 130, 246, 0.1);
  color: #3b82f6;
}
.provider-avatar.dashscope {
