...
export default {
  ...
  data() {
    return {
      ...
      // 移动中样式
      moveBlockBackgroundColor: 'var(--el-bg-color-overlay)',
      leftBarBorderColor: 'var(--el-border-color-light)',
      iconColor: 'var(--el-text-color-secondary)',
  ...
  methods: {
    init() {
      ...
      // 定义新的命名监听器
      this._touchmoveHandler = (e) => { this.move(e) }
      this._mousemoveHandler = (e) => { this.move(e) }
      this._touchendHandler = () => { this.end() }
      this._mouseupHandler = () => { this.end() }
    ...
    start: function(e) {
      ...
      if (this.isEnd == false) {
        this.text = ''
        this.moveBlockBackgroundColor = 'var(--el-color-primary)'
        this.leftBarBorderColor = 'var(--el-color-primary)'
        this.iconColor = '#fff'
        e.stopPropagation()
        this.status = true
      }
    ...
    // 鼠标松开
      ...
      // 判断是否重合
      if (this.status && this.isEnd == false) {
        this.status = false // 第一时间置为 false 阻断重复触发
        ...
        reqCheck(data).then(res => {
          if (res.repCode == '0000') {
            this.moveBlockBackgroundColor = 'var(--el-color-success)'
            this.leftBarBorderColor = 'var(--el-color-success)'
            this.iconColor = '#fff'
            ...
            setTimeout(() => {
              this.tipWords = ''
              if (this.$parent && typeof this.$parent.closeBox === 'function') {
                this.$parent.closeBox()
              }
              this.$emit('success', { captchaVerification })
            }, 1000)
          } else {
            this.moveBlockBackgroundColor = 'var(--el-color-danger)'
            this.leftBarBorderColor = 'var(--el-color-danger)'
            this.iconColor = '#fff'
    ...
    refresh: function() {
      ...
      this.leftBarBorderColor = 'var(--el-border-color-light)'
      this.moveBlockBackgroundColor = 'var(--el-bg-color-overlay)'
      this.iconColor = 'var(--el-text-color-secondary)'
