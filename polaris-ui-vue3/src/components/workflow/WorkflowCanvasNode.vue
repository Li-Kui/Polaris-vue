<template>
  ...
  >
    ...
    <div v-else-if="data.type === 'parallel'" class="loop-content">
      <small class="loop-mode-summary">{{ data.parallelSummary }}</small>
      <div v-for="branch in data.parallelBranches" :key="branch.port" class="loop-branch-row">
        <span>{{ branch.label }}</span>
        <small :class="{missing: !branch.connected}">{{ branch.targetName }}</small>
    ...
    <Handle v-if="!data.end && !['llm_classifier', 'loop', 'parallel', 'sub_workflow'].includes(data.type) && !(data.type === 'approval' && data.approvalBranches?.length)" type="source" :position="Position.Right" class="workflow-node-handle" />
  </article>
</template>
